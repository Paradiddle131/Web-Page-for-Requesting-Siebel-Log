    ...
    for machine_no in [str(x) for x in range(1, 15)]:
        try:
            ...
            last_update = datetime.fromtimestamp(
                int(servers[int(machine_no) - 1]["log_level_last_updated"].split('.')[0]))
            ...
            if servers[int(machine_no) - 1]["log_level_status"] == '5' and \
                ...
                info(f"Skipping. Log level has been 5 on machine #{machine_no} for {time_difference} minutes.")
                    ...
                    time_difference >= 30:
                with app.test_request_context('/', json=dumps(
                        {"machine_no": machine_no, "isAdm": False, "Server_action": "close_log"})):
                    siebel = Siebel(req_data=loads(request.json))
                siebel.change_log_level(Change_log_action.DECREASE)
                info(
                    f"Log level on machine #{machine_no} has been changed to 0 due to 30+ minutes of continuous activeness since: {last_update}.")
            else:
